...
//데이터베이스 통신 db 연결
    ...
    String sql = " SELECT * FROM posting INNER JOIN reply ON posting.posting_idx = reply.posting_idx";

//여기로 오기전에 post_idx를 받아와야함
// 조건추가 where 특정 게시글. post_idx 
...
<body>
    ...
    <%-- 여기는 html영역 쉽게 말해 jsp의 반복문의 내용으로 html tag를 넣어줄 수 있음 --%>
        <div> 제목:<%=result.getString(3) %> / 내용:<%=result.getString(4) %> </div>
        <div> 댓글 : <%=result.getString(9) %> / 댓글작성일 :<%=result.getString(10) %></div>
    <%
       }
    %>
